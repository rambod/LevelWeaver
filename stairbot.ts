...
import { snapshotCollisionBoxes, corridorWallCapsules, checkPlayerCollision, type CollisionWorld } from './src/playtest/collision/index.ts'
...
function walkStair(level: GeneratedLevel, world: CollisionWorld, tagged: { box: THREE.Box3; name: string }[], stairId: string): StairResult {
  ...
  const pushWp = (x: number, z: number): void => {
    wps.push({ x, z })
  }
  ...
  for (let i = 0; i < treads.length; i++) pushWp(treads[i].x, treads[i].z)
  pushWp(upper.position.x, upper.position.z)
  ...
  if (Math.abs(eye - wantEye) > 0.6) {
    return fail(`wrong-floor eye=${eye.toFixed(2)} want=${wantEye.toFixed(2)}`, eye)
  }
  ctrl.dispose?.()
